...
const $commaLike = (ctx: Context): string => {
  const { i, it } = ctx
...
const $logicalOperator = (ctx: Context): string => {
  const { value } = ctx.it
  return ['&&', '||'].includes(value) ? ` ${value} ` : value
}

const $negative = (ctx: Context): string => {
  const { value } = ctx.it
  return value === '-' ? value : ''
}
...
const $statement = (ctx: Context): string => {
  const { value } = ctx.it
...
const main = (ctx: _Context): string => {
  ...
  if (ctx.option.insertTranspilerInformation)
    content = ['; Generated by Coffee-AHK/0.0.43', content].join('\n')
